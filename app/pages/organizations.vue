...
watchEffect(() => {
  if (session.value?.data?.user) {
    console.log("[Better Auth] Session:", session.value.data.user.email)
  }
  if (organizations.value?.data) {
    console.log("[Better Auth] Organisations chargées:", organizations.value.data.length)
  }
  if (activeOrg.value?.data) {
    console.log("[Better Auth] Organisation active:", activeOrg.value.data.name)
  }
})
</script>
...
<template>
  <div style="padding: 20px; font-family: sans-serif; max-width: 900px; margin: 0 auto; line-height: 1.5;">
    ...
    <div v-if="session.isPending">Chargement de la session...</div>

    <div v-else-if="!session.data" style="color: red; padding: 20px; border: 2px solid red; border-radius: 8px; text-align: center;">
    ...
    <div v-else>
      <div style="display: flex; justify-content: space-between; align-items: center; background: #f0f4f8; padding: 10px 20px; border-radius: 8px; margin-bottom: 20px;">
        <span>Utilisateur : <strong>{{ session.data.user.email }}</strong></span>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
        ...
        <div>
          <section style="background: white; padding: 20px; border: 1px solid #ddd; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div v-if="organizations.isPending">Chargement...</div>
            <div v-else-if="organizations.error" style="color: red;">{{ organizations.error.message }}</div>
            <div v-else-if="!organizations.data || organizations.data.length === 0" style="color: #666;">Vous n'êtes membre d'aucune organisation.</div>
            <ul v-else style="list-style: none; padding: 0;">
              <li v-for="org in organizations.data" :key="org.id" style="display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid #eee;">
                <span><strong>{{ org.name }}</strong> ({{ org.slug }})</span>
                <button @click="switchOrg(org.id)" :disabled="activeOrg.data?.id === org.id || isLoading" :style="{ background: activeOrg.data?.id === org.id ? '#ddd' : '#2196f3', color: activeOrg.data?.id === org.id ? '#666' : 'white', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: activeOrg.data?.id === org.id ? 'default' : 'pointer' }">
                  {{ activeOrg.data?.id === org.id ? 'Active' : 'Activer' }}
                </button>
              </li>
            </ul>
          </section>

          <section v-if="activeOrg.data" style="background: #e8f5e9; padding: 20px; border: 1px solid #c8e6c9; border-radius: 8px;">
            <h2 style="margin-top: 0; color: #2e7d32;">Organisation Active : {{ activeOrg.data.name }}</h2>
